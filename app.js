...
/*************************************************
 * app.js — Nexus Churchs (FULL)
 * - Multi iglesias (URL ?church=ID)
 * - Tabs + deep links (#registro/#wow/#config/#historial)
 * - Registro realtime + WOW fullscreen + audio fade
 * - Sesiones (abrir/cerrar) + Historial + PDF
 * - PIN por iglesia (kiosk light)
 *************************************************/
...
window.addEventListener("hashchange", ()=>{
  openTab(location.hash, {syncHash:false});
});
...
function watchChurch(){
  ...
  unsubChurch = db.collection("churches").doc(churchId).onSnapshot(doc=>{
    ...
    // aplica lock por iglesia (solo si no está unlocked)
    enforcePinForChurch();
...
function bindActions(){
  if(btnSaveWow){
    btnSaveWow.onclick = async ()=>{
      ...
      if(wowSavedMsg) wowSavedMsg.textContent = `WOW guardado: ${sec}s (global en esta iglesia)`;
    };
  }
    ...
    btnCreateChurch.onclick = async ()=>{
      ...
      try{
        ...
        await db.collection("churches").doc(id).set({
          ...
          wowSeconds: 6,
          activeEventId: null,
          pinHash: "", // vacío => default 1234 hasta que lo cambies
  ...
  if(btnOpenSession) btnOpenSession.onclick = openSession;
  if(btnCloseSession) btnCloseSession.onclick = closeSession;

  if(formCheckin) formCheckin.onsubmit = submitCheckin;
}
...
/******** WOW pop: incluye nombre de iglesia ********/
  ...
  if(btnSavePin){
    btnSavePin.onclick = async ()=>{
      ...
      try{
        ...
        // actualiza local state inmediatamente
        ...
        console.error(e);
        if(pinSavedMsg) pinSavedMsg.textContent = "No se pudo guardar el PIN. Revisa reglas.";
      }
    };
  }
}
