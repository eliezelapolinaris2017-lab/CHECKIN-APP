...
/******** Iglesia name (para WOW) ********/
let churchName = "";  // <-- aquí guardamos el nombre real
let lastWelcomeText = "Bienvenidos";
...
/******** Watch church doc (nombre + settings) ********/
function watchChurch(){
  ...
  unsubChurch = db.collection("churches").doc(churchId).onSnapshot(doc=>{
    ...
    // ✅ nombre real de la iglesia para WOW
...
/******** WOW pop: premium copy ********/
function popWelcome(fullName){
  if(welcomeTimer) clearTimeout(welcomeTimer);

  // ✅ Copy más corto y elegante:
  const churchText = churchName ? ` — Iglesia ${churchName}` : "";
