...
/***********************
 * PIN POR IGLESIA (CORREGIDO)
 * - Default 1234 sin hashing (si no existe pinHash)
 * - Si existe pinHash: usa SHA-256, con try/catch
 ***********************/
function isUnlocked(){
...
async function tryUnlockPin(){
  try{
    ...
    // ✅ Sin pinHash => default 1234 directo (sin hashing)
    if(!churchPinHash){
    ...
    // ✅ Con pinHash => valida por SHA-256
    ...
    showPinError("Este navegador bloqueó la validación del PIN. Prueba Chrome o desactiva bloqueos.");
  }
}
...
function bindPinActions(){
  ...
  // guardar PIN por iglesia (requiere hashing, pero con try/catch)
  on(btnSavePin, "click", async (e)=>{
    ...
    try{
      ...
      // si no existía pinHash, el pin actual válido debe ser default 1234
      if(!churchPinHash){
        const defOk = (cur === DEFAULT_PIN);
        if(!defOk){
      ...
      if(pinSavedMsg) pinSavedMsg.textContent = "No se pudo guardar el PIN (crypto/subtle bloqueado).";
    }
  });
}
