...
/* ==========================================================
   Nexus Churchs — app.js (FULL) — BUILD: WOW-FIRESTORE-QUEUE
   ...
   - WOW pinta SOLO desde Firestore (no fetch a archivos individuales)
   - Sin ticker de nombres
   - Pausa de versículos cuando entra un check-in (anti-choque)
========================================================== */
...
const tickerTrack = document.getElementById("tickerTrack"); // lo vamos a desactivar
...
let WOW_QUEUE = [];      // [{ref,text,duration}]
let queueEnabled = false;
let queueIndex = 0;
let queueTimer = null;

/* Pause verses on new checkin */
    ...
    .onSnapshot(snap=>{
      ...
      if(newestId && newest && newest.fullName){
        if(firstLoad){
          ...
          // ✅ pausa versículos para que no choquen visualmente
          pauseVersesTemporarily(8);
        }
      }
    }, ()=>{});
}
...
/* =========================
   Pause verses temporarily (anti-choque)
========================= */
function pauseVersesTemporarily(sec){
  isVersePaused = true;
  renderCurrentQueueItem();
...
/* ==========================================================
   ...
   NOTA: la cola guarda TEXT/REF, NO usa path al proyectar.
========================================================== */
async function loadVersesCatalog(){
  ...
  try{
    ...
    VERSES_CATALOG.forEach((it, idx)=>{
      ...
      opt.textContent = `${clean(it.ref||"Verso")} — ${trunc(clean(it.text||""), 45)}`;
...
/* ==========================================================
   QUEUE — sincronizada por iglesia (FIRESTORE)
   churches/{churchId}:
   wowQueue: [{ref,text,duration}]
...
function applyQueueFromChurchDoc(d){
  ...
  if(queueEnabled) startQueueTimer();
  else stopQueueTimer();
...
function startQueueTimer(){
  stopQueueTimer();
  if(!queueEnabled || !WOW_QUEUE || WOW_QUEUE.length===0) return;

  const cur = WOW_QUEUE[queueIndex] || WOW_QUEUE[0];
  const sec = clamp(parseInt(cur?.duration || 12,10) || 12, 3, 120);
...
/*************** WOW RENDER SOLO DESDE FIRESTORE ****************/
...
function bindQueueActions(){
  if(btnAddToQueue){
    btnAddToQueue.onclick = async ()=>{
      ...
      // ✅ guardamos texto/ref en Firestore (no path)
      const item = { ref: clean(it.ref||""), text: clean(it.text||""), duration: dur };
      if(!item.text){
        queueMsg && (queueMsg.textContent = "Ese item no trae texto.");
        return;
      }
